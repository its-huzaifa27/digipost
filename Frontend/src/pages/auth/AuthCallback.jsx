...
export function AuthCallback() {
  ...
  useEffect(() => {
    ...
    if (code) {
      ...
      setStatus(
        `Facebook Error ${errorCode ? `(${errorCode})` : ""}: ${errorMsg}`,
      );
    } else {
  ...
  const connectFacebook = async (code) => {
    try {
      ...
      const selectedClient = selectedClientRaw
        ? JSON.parse(selectedClientRaw)
        : null;
      const clientId = selectedClient?.id;

      if (!clientId || clientId === "all") {
        setStatus(
          "Error: Please select a client before connecting social accounts.",
        );
        return;
      }
  ...
  return (
    <div className="flex items-center justify-center min-h-screen bg-gray-50">
      <Card className="p-8 text-center max-w-md w-full">
        ...
        <div
          className={`text-lg ${status.startsWith("Error") ? "text-red-600" : "text-blue-600"}`}
        >
          {status}
        </div>
      </Card>
