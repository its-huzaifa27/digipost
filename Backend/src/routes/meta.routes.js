...
router.post("/callback", authenticateToken, async (req, res) => {
  ...
  try {
    ...
    console.log(
      `[MetaCallback] Client Lookup Result:`,
...
router.post("/disconnect", authenticateToken, async (req, res) => {
  ...
  try {
    ...
    // If connectionId is provided, delete that specific one.
    if (connectionId) {
      query.id = connectionId;
    } else if (platform) {
      // If platform provided, delete all for that platform
      query.platform = platform;
    }
    // If NEITHER is provided, we default to deleting ALL connections for this client (Hard Reset)
    // This helps clear ghost records.
  ...
  try {
    ...
    if (connection.platform === "facebook") {
      ...
      return res.status(400).json({
        error: `Analytics not supported for platform: ${connection.platform}`,
      });
    }
