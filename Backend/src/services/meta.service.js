...
class MetaService {
  ...
  async exchangeCodeForToken(code, userId, clientId = null) {
    try {
      ...
      if (tokenResponse.data.error) {
        ...
        throw new Error(
          `Facebook OAuth Error: ${tokenResponse.data.error.message || "Failed to exchange code for token"}`,
        );
      }
      ...
      if (longLivedResponse.data.error) {
        console.error(
          "Facebook Long-Lived Token Exchange Error:",
          longLivedResponse.data.error,
        );
        throw new Error(
          `Facebook OAuth Error: ${longLivedResponse.data.error.message || "Failed to exchange for long-lived token"}`,
        );
      }
      ...
      const connectedPagesCount = await this.fetchAndSavePages(
        longLivedToken,
        userId,
        clientId,
      );

      return { success: true, connectedPagesCount };
    } catch (error) {
      console.error(
  ...
  async fetchAndSavePages(userAccessToken, userId, clientId = null) {
    try {
      ...
      if (response.data.error) {
        ...
        throw new Error(
          `Facebook API Error: ${response.data.error.message || "Unknown error"}`,
        );
      }
      ...
      if (!Array.isArray(pages)) {
        console.error(
          "Unexpected Facebook API response structure:",
          response.data,
        );
      ...
      if (pages.length === 0) {
        ...
        return 0; // No pages to save, but this is not an error
      }
  ...
  async publishToFacebook(connection, message, mediaUrls, scheduledTime) {
    try {
        ...
        const mediaIds = await Promise.all(
          media.map(async (img) => {
            ...
            const res = await axios.post(
              `${FB_GRAPH_URL}/${connection.pageId}/photos`,
              uploadParams,
            );
            return res.data.id;
          }),
      ...
      console.error(
        `FB Publish Error (${connection.pageName}):`,
        error.response?.data || error.message,
      );
      throw error;
    }
  ...
  async publishToInstagram(connection, caption, mediaUrls) {
    try {
      ...
      console.error(
        `IG Publish Error (${connection.pageName}):`,
        error.response?.data || error.message,
      );
      throw error;
    }
  ...
  async _waitForInstagramMedia(creationId, accessToken) {
    ...
    while (status === "IN_PROGRESS" && attempts < maxAttempts) {
      ...
      try {
        ...
        console.log(
          `Instagram Media Status (${attempts}/${maxAttempts}):`,
          status,
        );
        ...
        console.warn(
          "Failed to check IG status, retrying...",
          statusError.message,
        );
      }
    }
    ...
    if (status !== "FINISHED") {
      throw new Error(
        "Instagram Media timed out processing. The image might be too large or invalid.",
      );
    }
  }
  ...
  async getInstagramInsights(connection) {
    ...
    try {
      if (!connection.igBusinessId)
        throw new Error("No IG Business ID linked.");

      // 1. Fetch Profile
      let profileData = {};
      try {
        const profileRes = await axios.get(
          `${FB_GRAPH_URL}/${connection.igBusinessId}`,
          {
            params: {
              fields:
        ...
        console.error("[IG_INSIGHTS] Profile Fetch Error:", err.message);
      }
      ...
      const fetchAccountMetric = async (metric, type = null) => {
        try {
          ...
          const res = await axios.get(
            `${FB_GRAPH_URL}/${connection.igBusinessId}/insights`,
            { params },
          );
      ...
      insightsData.push(
        ...(await fetchAccountMetric(
          "views,total_interactions,accounts_engaged,profile_views,website_clicks",
          "total_value",
        )),
      );
      ...
      if (
        ...
        profileData.followers_count >= 100
      ) {
        try {
          ...
          try {
            ...
            console.error(
              "[IG_INSIGHTS] Follower Demographics failed:",
          ...
          try {
            const engagedRes = await axios.get(
              `${FB_GRAPH_URL}/${connection.igBusinessId}/insights`,
              {
                params: {
            ...
            console.log(
              "[IG_DEBUG] Engaged Raw:",
              JSON.stringify(engagedRes.data, null, 2),
            );
            insightsData.push(...(engagedRes.data.data || []));
          } catch (e) {
            console.error(
              "[IG_INSIGHTS] Engaged Demographics failed:",
              e.response?.data?.error?.message,
            );
            errors.engaged_audience_demographics =
              e.response?.data?.error?.message;
          }

          // 3. Reached Audience (Last 30 Days) - Requested Fix
          try {
            const reachedRes = await axios.get(
              `${FB_GRAPH_URL}/${connection.igBusinessId}/insights`,
              {
                params: {
            ...
            console.log(
              "[IG_DEBUG] Reached Raw:",
              JSON.stringify(reachedRes.data, null, 2),
            );
            insightsData.push(...(reachedRes.data.data || []));
          } catch (e) {
            console.error(
              "[IG_INSIGHTS] Reached Demographics failed:",
              e.response?.data?.error?.message,
            );
            errors.reached_audience_demographics =
              e.response?.data?.error?.message;
          }
        } catch (e) {
        ...
        console.log(
          `[IG_INSIGHTS] Skipping demographics: Not enough followers (${profileData?.followers_count || 0} < 100)`,
        );
      }
      ...
      try {
        ...
        const mediaResponse = await axios.get(
          `${FB_GRAPH_URL}/${connection.igBusinessId}/media`,
          {
            params: {
              fields:
                "id,media_type,media_product_type,media_url,thumbnail_url,timestamp,caption,like_count,comments_count,ig_play_count,permalink",
              limit: 100,
              access_token: connection.accessToken,
            },
          },
          ...
          mediaList.map(async (media) => {
            ...
            try {
              const viewsRes = await axios.get(
                `${FB_GRAPH_URL}/${media.id}/insights`,
                {
                  params: {
                    metric: "views",
                    access_token: connection.accessToken,
        ...
        console.error(
          "[IG_INSIGHTS] Fatal Media List Error:",
          mediaErr.message,
        );
        return {
          profile: profileData,
          insights: insightsData,
          topMedia: topMedia,
          publishingSummary: { total: 0, videos: 0, photos: 0, carousels: 0 },
          errors: { media: mediaErr.message },
        };
      }
    } catch (error) {
  ...
  async getFacebookInsights(connection) {
    console.log(
      `[FB_INSIGHTS] Starting fetch for: ${connection.pageName} (${connection.pageId})`,
    );
    ...
    try {
      ...
      try {
        const pageResponse = await axios.get(
          `${FB_GRAPH_URL}/${connection.pageId}`,
          {
            params: {
        ...
        console.log(
          `[FB_INSIGHTS] Profile fetch success: ${profileData.followers_count} likes`,
        );
      } catch (profileErr) {
        console.error(
          `[FB_INSIGHTS] Profile fetch failed:`,
          profileErr.response?.data || profileErr.message,
        );
        throw new Error(
          `Facebook Profile Error: ${profileErr.response?.data?.error?.message || profileErr.message}`,
        );
      }
      ...
      try {
        const insightsResponse = await axios.get(
          `${FB_GRAPH_URL}/${connection.pageId}/insights`,
          {
            params: {
              ...
              metric:
                "page_impressions_unique,page_daily_active_users,page_impressions,page_fan_adds,page_post_engagements,page_impressions_by_is_fan_unique",
              period: "day",
              access_token: connection.accessToken,
            },
          },
        ...
        console.warn(
          `[FB_INSIGHTS] Full set fetch failed:`,
        ...
        try {
          console.log(
            `[FB_INSIGHTS] Attempting fallback fetch (page_impressions only)`,
          );
          ...
          console.error(
            `[FB_INSIGHTS] Fallback fetch failed:`,
      ...
      try {
        const splitMetric = insightsData.find(
          (m) => m.name === "page_impressions_by_is_fan_unique",
        );
        if (
          splitMetric &&
          splitMetric.values &&
          splitMetric.values[0] &&
          splitMetric.values[0].value
        ) {
        ...
        console.warn(
          "[FB_INSIGHTS] Failed to process audience split:",
          splitErr.message,
        );
      }
      ...
      try {
        const feedResponse = await axios.get(
          `${FB_GRAPH_URL}/${connection.pageId}/feed`,
          {
            params: {
              fields:
                "id,message,created_time,full_picture,likes.summary(true),comments.summary(true),permalink_url",
              limit: 100,
              access_token: connection.accessToken,
            },
          },
        ...
        console.log(
          `[FB_INSIGHTS] Posts fetch success: ${topMedia.length} posts`,
        );
      } catch (feedErr) {
        console.warn(
          `[FB_INSIGHTS] Posts fetch failed:`,
      ...
      const publishingSummary = {
        ...
        carousels: topMedia.filter((m) => m.media_type === "CAROUSEL_ALBUM")
          .length,
      };
